			...
			case SCROLL_CARD_SELECT: {
				...
				for(int i = 0; i < 5; ++i) {
					...
					if(selectable_cards[i + pos]->code)
						...
						mainGame->btnCardSelect[i]->setImage(imageManager.tCover[pcard->controler]);
			...
			case SCROLL_CARD_DISPLAY: {
				...
				for(int i = 0; i < 5; ++i) {
					...
					if(display_cards[i + pos]->code)
						...
						mainGame->btnCardDisplay[i]->setImage(imageManager.tCover[selectable_cards[i]->controler + 2]);
