...
bool ClientField::OnEvent(const irr::SEvent& event) {
	...
	case irr::EET_MOUSE_INPUT_EVENT: {
		...
		case irr::EMIE_RMOUSE_LEFT_UP: {
			irr::core::position2di pos = mainGame->ResizeReverse(event.MouseInput.X, event.MouseInput.Y);
...
bool ClientField::OnCommonEvent(const irr::SEvent& event) {
	...
	case irr::EET_KEY_INPUT_EVENT: {
		...
		case irr::KEY_KEY_V: {
			...
			if(focus && focus->getType() == irr::gui::EGUIET_EDIT_BOX && event.KeyInput.Control) {
	...
	case irr::EET_MOUSE_INPUT_EVENT: {
		switch(event.MouseInput.Event) {
		case irr::EMIE_LMOUSE_PRESSED_DOWN: {
			auto* guiRoot = mainGame->env->getRootGUIElement();
			auto mousepos = irr::core::position2di(event.MouseInput.X, event.MouseInput.Y);
			if(guiRoot->getElementFromPoint(mousepos) == mainGame->stText) {
				if(!mainGame->scrCardText->isVisible()) {
					break;
				}
